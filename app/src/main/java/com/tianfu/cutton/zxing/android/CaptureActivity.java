package com.tianfu.cutton.zxing.android;
...
import com.tianfu.cutton.activity.login.ScanLoginActivity;
import com.tianfu.cutton.activity.quality.BagDetailsActivity;
...
public final class CaptureActivity extends Activity implements
    ...
    private void goLogin() {
        ...
        map.put("uid",rawText.split("@")[0]);
        if (isLogin){
            HttpManager.getServerApi().scan(map).enqueue(new CallBack<BagDetailsMessageBean>() {
                @Override
                public void success(BagDetailsMessageBean data) {
                        ...
                        intent.setClass(BaseApplication.getContextObject(), ScanLoginActivity.class);
    ...
    private void goTiaoxing(Result rawResult) {
        Map<String, String> map = new HashMap<>();
        map.put("barCode", rawResult.getText());
        HttpManager.getServerApi().getBag(map).enqueue(new CallBack<BagDetailsMessageBean>() {
            @Override
            public void success(BagDetailsMessageBean data) {
                if (data!=null&&data.success) {
                    BagDetailsMessageBean.ValueBean valueBean = data.value;
                    Intent intent = new Intent(CaptureActivity.this, BagDetailsActivity.class);
                    intent.putExtra("code", data.value.batchCode);
                    intent.putExtra("fromKun", "0");
                    intent.putExtra("productId", data.value.id);
